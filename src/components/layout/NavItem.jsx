        ...
        <div>
            <div className='relative' onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
                ...
                {each.subNav && showDropdown && (
                    <ul className='absolute flex flex-col bg-primary text-alt  w-auto top-[30px] z-1 p-4 transition-all duration-[5000ms] shadow' style={{ opacity: showDropdown ? 1 : 0, transition: 'opacity 0.3s ease-in-out' }}>
                        {each.subNav.map((sub) => {
                            return (
